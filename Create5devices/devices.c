...
static int cntr = 0; // scope is limited until the lifecycle of the program i.e. until the program gets terminated
static int __init my_mod_init(void) // scope is limited until the function in this particular file
{
    int ans, i, major, minor, j;
    char dev_string[20] = {0}; // Increased size to accommodate new device names
    ...
    for (i = 0; i < NUM_DEVS; i++)
    {
        ...
        // Creating device names: my_deviceA, my_deviceB, etc.
        sprintf(dev_string, "%s%c", MY_DEV_NAME, 'A' + i); 
    ...
    pr_info("%d devices under class %s created with success\n",
            i, MY_CLASS_NAME);
    return 0;
