...
// sysfs attr variable
struct kobj_attribute my_sysfs_attr = __ATTR(cdac_int, 0660, my_sysfs_show, my_sysfs_store);
// ---- sysfs related ----
...
static int __init my_mod_init(void)
{
	...
	if (kobj_ref==NULL)
	{
		pr_err("cdac_sys dir %s could not be created!\n", MY_SYSFS_DIR);
		goto r_sysfs_dir;
	}
	...
	{
		pr_err("Cannot create cdac_int file\n");
		goto r_sysfs_file;
	}

	pr_info("cdac_sys directory %s created under /sys/kernel\n", MY_SYSFS_DIR);
